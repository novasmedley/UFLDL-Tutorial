...
cost = (1/m)*(sum(sum(0.5*((h-data(1:visibleSize,:)).^2)))) + decay + penalty; % total cost


%-------BACKWARD--------
sSpar = beta*( (-sparsityParam./p) + ((1-sparsityParam)./(1-p)) ); % sparsity penalty component of the derivative
sSpar = repmat(sSpar,1,m);

% "error term" for how "responsible" a node is to the output
% using just the weights, not the bias terms
s2 = -(data(1:visibleSize,:)-h); % for LINEAR
